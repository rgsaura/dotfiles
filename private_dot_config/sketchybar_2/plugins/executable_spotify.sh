...
repeat () 
{
  ...
  if [ "$REPEAT" = "false" ]; then
    sketchybar_2 -m --set spotify.repeat icon.highlight=on
    osascript -e 'tell application "Spotify" to set repeating to true'
  else 
    sketchybar_2 -m --set spotify.repeat icon.highlight=off
...
shuffle () 
{
  ...
  if [ "$SHUFFLE" = "false" ]; then
    sketchybar_2 -m --set spotify.shuffle icon.highlight=on
    osascript -e 'tell application "Spotify" to set shuffling to true'
  else 
    sketchybar_2 -m --set spotify.shuffle icon.highlight=off
...
scrubbing() {
  ...
  sketchybar_2 --set spotify.state slider.percentage=$PERCENTAGE
}
...
scroll() {
  ...
  sketchybar_2 --animate linear 10 \
...
popup () {
  sketchybar_2 --set spotify.anchor popup.drawing=$1
}
