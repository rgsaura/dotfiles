...
update() {
  ...
  PREV_COUNT=$(sketchybar_2 --query github.bell | jq -r .label.value)
  ...
  do
    ...
    notification=(
      ...
      drawing=on
      click_script="open $URL; sketchybar_2 --set github.bell popup.drawing=off"
    )
  ...
  if [ $COUNT -gt $PREV_COUNT ] 2>/dev/null || [ "$SENDER" = "forced" ]; then
    sketchybar_2 --animate tanh 15 --set github.bell label.y_offset=5 label.y_offset=0
  fi
}

popup() {
  sketchybar_2 --set $NAME popup.drawing=$1
}
